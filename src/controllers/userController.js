...
async function loginAdmin(req, res) {
    try {
      ...
      // Generate JWT
      const token = jwt.sign({ adminId }, 'your_secret_key');
...
async function registerDealership(req, res) {
  try {
...
// Dealership login
async function loginDealership(req, res) {
  try {
   ...
   if (!isMatch) {
    ...
    // Generate JWT
    const token = jwt.sign({ dealershipEmail }, 'your_secret_key');
    res.status(200).json({ token });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal Server Error' });
